...
reserve_path_info_link(struct capvm *vm, uint32_t *pi_id)
{
	if (vm->paths.live == vm->paths.ceil) {
		...
		if (nceil > 1000) {
			/* FIXME: this can legitimately happen due to very long
			 * inputs, but with a shorter input it's a sign
			 * of something getting out of hand.
			 *
			 * Remove this assert once the path table is a
			 * fixed size, or calculate an appropriate limit
			 * based on regex/program and input length. */
			assert(!"path table > 1,000");
		}
...
#define LOG_CMP_PATHS 0
	...
	while (link_a != NO_ID) {
		...
		hash_a_before += fsm_hash_id(link_a + hash_step);
		link_a = prev;
		hash_step++;
	}
	hash_step = 0;
	while (link_b != NO_ID) {
		...
		hash_b_before += fsm_hash_id(link_b + hash_step);
		link_b = prev;
		hash_step++;
	}
	...
	while (link_a != NO_ID) {
		assert(link_a < vm->paths.ceil);
		const uint32_t prev = get_path_node_backlink(vm, link_a);
		hash_a_after += fsm_hash_id(link_a + hash_step);
		link_a = prev;
		hash_step++;
	}
	link_b = p_b;
	...
	while (link_b != NO_ID) {
		assert(link_b < vm->paths.ceil);
		const uint32_t prev = get_path_node_backlink(vm, link_b);
		hash_b_after += fsm_hash_id(link_b + hash_step);
		link_b = prev;
		hash_step++;
	}
    ...
    uint32_t path_info_head, uint32_t uniq_id)
{
	...
	if (already_evaluated) {
		 ...
		 *
		 * FIXME: Ideally, avoid the linear scan here, but the
		 * total stack height is bounded by the generated program size
		 * and should be fairly small in practice.
		 *
...
alloc_error:
	release_path_info_link(vm, &path_info_head);
	/* FIXME: return error or set error flag on vm */
}
    ...
    struct fsm_capture *capture_buf)
{
	...
	/* FIXME: This can grow beyond this. In practice it will usually
	 * stay fairly small, but quantify this better. The worst case
	 * should be decidable based on the compiled program and input
	 * length. */
	const size_t path_info_max = 2 * thread_max; /* TBD. < 3x? */
	...
	/* FIXME: This needs a bounds check. Change the the API so the caller
	 * passes in the total capture buf size. */
	...
	if (eval_vm(&vm)) {
		...
		/* FIXME: this assert will not work if refcounts are sticky at the max value,
		 * but if the number of paths and threads is bounded then it shouldn't be
		 * possible to overflow the refcount anyway. */
		assert(vm.paths.live == 0);
	}
	TIME(&post);
