...
fsm_exec_with_captures(const struct fsm *fsm, const unsigned char *input,
	size_t input_length, fsm_state_t *end, struct fsm_capture *captures)
{
	fsm_state_t state;
	size_t offset = 0;
	...
	if (captures != NULL) {
		/* FIXME: should use a passed-in buffer size, and check
		 * that capture_ceiling fits. */
