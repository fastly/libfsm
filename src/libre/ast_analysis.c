...
/* FIXME: avoid the double negative here? */
...
struct anchoring_env {
	...
	/* Special case for '$[^a]', which matches "\n" with a capture
	 * group 0 of (0,1) in PCRE. */
...
ast_analysis(struct ast *ast, enum re_flags flags)
{
	 ...
	 */
	{
		...
		/* second anchoring pass, sweeping backward */
