    ...
    uint32_t *id)
{
	...
	if (env->charclass_htab.buckets_used >= env->charclass_htab.bucket_count/2) {
		...
		for (uint32_t o_i = 0; o_i < ocount; o_i++) {
			...
			const uint64_t h = fsm_hash_fnv1a_64((const uint8_t *)obuckets[o_i].bitset.octets,
	...
	const uint64_t h = fsm_hash_fnv1a_64((const uint8_t *)chars,
	...
	struct capvm_program *p, const struct ast_expr *expr)
{
	...
	case AST_EXPR_ALT:
	{
		 ...
		 *
		 * `(x|(x|))^` is a case that triggers this.
		 * */
	...
	const struct ast *ast)
{
	...
	/* FIXME: it may be worth exposing these static checks as
...
#define DUMP_AST 0
#define DUMP_RESULT 1		/* should be 0 in production */
