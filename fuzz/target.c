...
#if EXPENSIVE_CHECKS
/* 60 seconds */
#define TIMEOUT_USEC (60ULL * 1000 * 1000)
#else
/* 10 seconds */
...
/* FIXME: move to common test util file */
#if 1
static void
...
{
	/* Skipping codegen for now -- We know this can become very slow
	 * with larger automata, and it shouldn't slow down fuzzer
	 * exploration. */
	return 1;
...
#define MAX_PATTERN_LEN 256
	...
	for (size_t i = 0; i < count; i++) {
		...
		struct fsm *fsm = re_comp(RE_PCRE, scanner_next, &s, &options, flags, &err);
		if (fsm == NULL) {
...
read_options_from_env(void)
{
	...
	if (!checked) {
		...
		if (mode_str != NULL) {
			switch (mode_str[0]) {
			case 'r': case 'R':
				mode = MODE_REGEX; /* default */
				break;
			case 's': case 'S':
				mode = MODE_REGEX_SINGLE_ONLY;
				break;
			case 'm': case 'M':
				mode = MODE_REGEX_MULTI_ONLY;
				break;
			case 'i': case 'I':
				mode = MODE_IDEMPOTENT_DET_MIN;
				break;
			default:
...
harness_fuzzer_target(const uint8_t *data, size_t size)
{
	...
	for (size_t i = 0; i < size; i++) {
		...
		if (c == '.') {
			dot_count++;
			if (dot_count >= 4) {
				/* too many '.'s can make it slow, but that's
				 * uninteresting to this particular fuzzer */
				return EXIT_SUCCESS;
			}
		}
