...
fuzz_eager_endids(const uint8_t *data, size_t size)
{
	...
	int ret = 0;
	const bool minimise = false;

	size_t max_pattern_length = 0;

	/* chop data to a series of patterns */
	{
		size_t prev = 0;
		size_t offset = 0;

		/* Patterns with lots of '.' can take a while to determinise.
		 * That which slows down fuzzer coverage but isn't interesting here. */
		size_t dots = 0;
...
#define MAX_DOTS 4
	...
	{
		size_t used = 0;
		struct fsm *copies[MAX_PATTERNS] = {0};
		...
		/* consumes copies[] */
		struct fsm *fsm = fsm_union_array(used, copies, NULL);
		assert(fsm != NULL);
